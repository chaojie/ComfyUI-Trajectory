...
const editor_path = "./extensions/ComfyUI-Trajectory/page/index.html"
...
app.registerExtension({
  ...
  nodeCreated(node) {
    ...
    if (title==="Edit Trajectory In Another Tab") {
      initEditorNode(node);
...
function initEditorNode(node)
{
  ...
  node.trajectoryWidget=node.widgets[2];
  node.canvasWidget=node.widgets[3];
  node.maskWidget=node.widgets[4];
...
async function loadArrayIntoWidget(widget, arr) {
  console.log(widget);
  widget.value = JSON.stringify(arr);
  app.graph.setDirtyCanvas(true);
}
...
async function messageFromEditor(event) {
  let nodes =  app.graph.findNodesByType("Trajectory_Canvas_Tab");
  //send same thing to all of the Trajectory_Canvas_Tab nodes
  const {title, selected} = event.data;
  
  console.log({title,selected});
  if (title) {
    
    let targetNodes = nodes.filter(node=>node.title==title || (node.title=="Edit Trajectory In Another Tab" && selected));
    nodes=targetNodes;
  }
  let queue = false;
  ...
  if (Array.isArray(event.data.trajectory)) {
    for (const node of nodes) {
      await loadArrayIntoWidget(node.trajectoryWidget,event.data.trajectory);
      if (node.triggerQueue.value) queue=true;
    }
  }
  if (queue) {
